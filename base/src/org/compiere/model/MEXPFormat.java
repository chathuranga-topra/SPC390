...
public class MEXPFormat extends X_EXP_Format {
	...
	protected boolean afterSave (boolean newRecord, boolean success){
		...
		//creating format lines based in the table
		if(!newRecord && getFormatLines().size() ==0){
			MTable mTable = (MTable) getAD_Table();
			List<MColumn> columns = new Query(getCtx(), MColumn.Table_Name, "AD_Table_ID = ?", get_TrxName())
			.setParameters(mTable.get_ID())
			...
			for(MColumn c : columns.toArray(new MColumn[columns.size()])){
				...
				expl.setIsMandatory(false);
				//set primary kesys
				if(c.getColumnName().equalsIgnoreCase(mTable.getTableName()+"_ID")){
					expl.setIsMandatory(true);
