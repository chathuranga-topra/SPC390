...
public class ImportHelper {
		...
		throws Exception, SQLException,	XPathExpressionException 
	{
		...
		MOrg org = new MOrg(ctx, po.getAD_Org_ID(), trxName);
		if(!org.isActive()){
			po = null;
			return;
		}
		...
		System.out.println(po.getAD_Client_ID());
			...
			throws Exception, XPathExpressionException
	{
		...
		for (MEXPFormatLine formatLine : formatLines) 
		{
			...
			else if(formatLine.getAD_Column().getAD_Reference_ID() == 15 && value.equals("")){
				System.out.println("Here I was called!");
				continue;
			}
			//Set the value
